function [SwarmRBF,DeltaRBF] = SLPSOMSE(net,Demons,SwarmRBF,DeltaRBF,Problem,Kriging_dmse)
...
%--------------------------------------------------------------------------
    ...
    for i = 1 : N
        ...
        if ~isempty(BetterInd)
            SwarmMean = mean(SwarmRBF(:,1:D),1);
            DeltaRBF(i,:) = rand(1,D).*DeltaRBF(i,:) + rand(1,D).*(ChoseId-SwarmRBF(i,1:D))+Kriging_dmse(i,:);
            %+rand(1,D).*(SwarmMean-SwarmRBF(i,1:D))
            % Update
